...
do $$ begin
  create type public.document_type as enum ('quote', 'proforma');
...
declare
  ...
  candidate text;
begin
  prefix := case when p_type = 'quote' then 'PREV' else 'PRO' end;
...
grant select, insert, update, delete on public.clients, public.jobs, public.documents, public.document_items to authenticated;
