...
const emptyClient: ClientInput = {
  ...
  address: '', postal_code: '', city: '', province: '', notes: '',
}
...
export function ManagementClientsPage() {
  ...
  const loadClients = async () => {
    const { data, error: loadError } = await supabase.from('clients').select('*').order('company_name').order('contact_name')
  ...
  const handleDelete = async (client: Client) => {
    ...
    if (deleteError) setError('Il cliente ha lavori o documenti collegati e non può essere eliminato.')
      ...
      <section className="management-card">
        ...
        {loading ? <div className="management-empty">Caricamento…</div> : filteredClients.length === 0 ? (
          ...
          <div className="management-table-wrap"><table className="management-table">
            <thead><tr><th>Cliente</th><th>Contatti</th><th>Località</th><th>Dati fiscali</th><th aria-label="Azioni" /></tr></thead>
            <tbody>{filteredClients.map((client) => <tr key={client.id}>
              <td><strong className="management-table__main">{clientDisplayName(client)}</strong>{client.company_name && client.contact_name && <small>{client.contact_name}</small>}</td>
      ...
      {formOpen && <div className="management-modal" role="dialog" aria-modal="true" aria-labelledby="client-form-title">
        ...
        <div className="management-modal__panel">
          ...
          <form className="management-form management-form--grid" onSubmit={(event) => void handleSubmit(event)}>
            ...
            <label><span>Partita IVA</span><input value={form.vat_number} onChange={(e) => setField('vat_number', e.target.value)} /></label>
            <label><span>Codice fiscale</span><input value={form.tax_code} onChange={(e) => setField('tax_code', e.target.value)} /></label>
            ...
            <label><span>CAP</span><input inputMode="numeric" value={form.postal_code} onChange={(e) => setField('postal_code', e.target.value)} /></label>
            <label><span>Città</span><input value={form.city} onChange={(e) => setField('city', e.target.value)} /></label>
